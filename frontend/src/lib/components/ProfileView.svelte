...
<div class="mx-auto grid max-w-5xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-2 lg:px-8">
	...
	<div class="flex flex-col gap-8">
		...
		{#if exams.length > 0}
			<section>
				<h2 class="text-lg font-semibold text-gray-900">Examens</h2>
				<table class="mt-4 w-full text-sm">
					<tbody>
						{#each exams as exam (exam.id)}
							<tr class="border-b border-gray-100 last:border-0">
								<td class="py-2 pr-4 font-medium text-gray-900">{exam.examTypeName}</td>
								<td class="py-2 text-gray-500">{examDateFormatter.format(exam.obtainedAt)}</td>
							</tr>
						{/each}
					</tbody>
				</table>
