...
- `Bluewater.Core/Services/Mail/BlueUserMergeTokenFactory.cs` — the one place that knows how to turn a `BlueUser` into the standard token set: `FirstName`, `FullName`, `Email`, `AddressBlock`. Every real-`BlueUser` recipient in both mail paths goes through this factory.
